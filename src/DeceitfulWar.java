...
public class DeceitfulWar {
    ...
    for (int i = 1; i <= t; i++) {
        ...
        ArrayList naomiBlocks = new ArrayList();
        for (int j = 0; j < numberOfBlocks; j++) {
            naomiBlocks.add(Double.parseDouble(line[j]));
        }
        Collections.sort(naomiBlocks);

        line = scanner.nextLine().split(" ");
        ArrayList kenBlocks = new ArrayList();
        for (int j = 0; j < numberOfBlocks; j++) {
            kenBlocks.add(Double.parseDouble(line[j]));
        }
        Collections.sort(kenBlocks);

        int result1, result2;

        result1 = optimalDeceitfulWar((List)naomiBlocks.clone(), (List)kenBlocks.clone());
        result2 = optimalWar((List)naomiBlocks.clone(), (List)kenBlocks.clone());

        out.println("Case #" + i + ": " + result1 + " " + result2);
    }

    out.close();
    }

    private static int optimalWar(List naomi, List ken) {
        int wins = 0;
        while (naomi.size() > 0){
            boolean found = false;
            double naomiDiscard = (Double)naomi.get(naomi.size() - 1);
            naomi.remove(naomi.size() - 1);
            for (int i = 0; i < ken.size(); i++) {
                if ((Double)ken.get(i) > naomiDiscard){
                    ken.remove(i);
                    found = true;
                    break;
                }
    ...
    private static int optimalDeceitfulWar(List naomi, List ken) {
        int wins = 0;
        while (naomi.size() > 0){
            boolean found = false;
            double naomiDiscard;
            if ((Double)naomi.get(0) > (Double)ken.get(0)){
                naomiDiscard = (Double)ken.get(ken.size()-1) + 0.0000001;
                naomi.remove(0);
            }
            else {
                naomiDiscard = (Double)ken.get(ken.size() - 1) - 0.0000001;
